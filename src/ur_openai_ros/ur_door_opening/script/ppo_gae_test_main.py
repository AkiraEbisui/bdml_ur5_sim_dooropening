...
def sub_program(save_steps, env):
...
#                print("arr_x_data_success_hist", arr_x_data_success_hist) # array([[-0.97627008,  1.24825577, -1.36089122], [-0.97627008,  1.24825577, -1.36089122],[-0.97627008,  1.24825577, -1.36089122]])
    ...
    with open('results/' + sys.argv[1] + '/result/sample-' + str(save_steps) + '.csv', 'w') as f:
        writer = csv.writer(f)
...
def main():
    ...
    for save_steps in range(100, 1500, 100):
        agent.saver.restore(agent.sess, './results/' + sys.argv[1] + '/ppo_with_gae_model-' + str(save_steps))
        env.success_times = 0
        success_rate_sub = sub_program(save_steps, env)
        success_sub_list.append(success_rate_sub)
