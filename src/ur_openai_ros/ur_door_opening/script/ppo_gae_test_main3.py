...
#                arr_x_data_fail_hist = np.array(x_data_fail_hist)
 ...
 #               arr_y_data_fail_hist = np.array(y_data_fail_hist) 
        ...
        if (update%1) == 0:
            ...
            plt.pause(1e-17)
            if test == 0:
                plt.savefig('./results/' + sys.argv[1] + '_ppo_with_gae_list.png')
            elif test == 1:
                plt.savefig('./results/' + sys.argv[1] + '_ppo_with_gae_list.png')

    if test == 0:
        with open('results/' + sys.argv[1] + '_sample.csv', 'w') as f0:        
        ...
        with open('results/' + sys.argv[1] + '_sample.csv', 'w') as f1:
...
#        agent.saver.restore(agent.sess, './results/' + sys.argv[1] + '/ppo_with_gae_model-' + str(save_steps))
    env.success_times = 0
    success_rate_sub1 = sub_program(env, 1)
    success_sub_list.append(success_rate_sub1)
    file_list.append(0)

    with open('results/' + sys.argv[1] + '_summary.csv', 'w') as f2:
