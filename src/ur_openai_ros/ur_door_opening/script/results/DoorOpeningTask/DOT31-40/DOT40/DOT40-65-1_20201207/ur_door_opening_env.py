...
#        rospy.logdebug("UR step func")	# define the logger
        ...
        for x in range(1, sub_step + 1):
            ...
            if self.moveit == 0:
                sub_action[0] = sub_action[0] / 60
                sub_action[1] = sub_action[1] / 15
                sub_action[2] = sub_action[2] / 15
                sub_action[3] = sub_action[3] / 50
                sub_action[4] = sub_action[4] / 50
...
#            print("min, max taxel", self.min_static_taxel0, self.max_static_taxel0, self.min_static_taxel1, self.max_static_taxel1)
    ...
    def compute_dist_rewards(self, action, update, observation):
        ...
        panel_c = 50000     #3 door panel open(+)
        ...
        if self.imu_link_rpy.x < 0:
            ...
            print("##1 reward_knob_rotation_r",self.knob_rotation_r)
            ...
            self.panel_rotation_r =  (1.5708061 - self.imu_link_rpy.z) * panel_c
            print("##1 reward_knob_rotation_r",self.knob_rotation_r, self.panel_rotation_r)
        elif 0.7 < self.imu_link_rpy.x:
            self.knob_rotation_r = 0.7 * knob_c + knob_bonus_c * 4
            self.panel_rotation_r =  (1.5708061 - self.imu_link_rpy.z) * panel_c
            print("##1 reward_knob_rotation_r", self.knob_rotation_r, self.panel_rotation_r)
        ...
        if abs(self.door_frame.position.x + 0.0659) > self.tolerances or abs(self.door_frame.position.y - 0.5649) > self.tolerances or abs(self.door_frame.position.z - 0.0999) > self.tolerances:
            self.door_frame_limit_r = - ( tolerances_c * ( n_step - update ) * 0.1 + tolerances_c)
        ...
        #6 over force limit1(-)
        if self.force_limit < self.delta_force_x or self.delta_force_x < -self.force_limit:
        	force_x_limit_r = - ( force_c * abs(abs(self.delta_force_x) - abs(self.force_limit)) * ( n_step - update ) * 0.1 + force_c)
                print("##3 force_x limit_r", force_x_limit_r)
        if self.force_limit < self.delta_force_y or self.delta_force_y < -self.force_limit:
        	force_y_limit_r = - ( force_c * abs(abs(self.delta_force_y) - abs(self.force_limit)) * ( n_step - update ) * 0.1 + force_c)
                print("##3 force_y limit_r", force_y_limit_r)
        if self.force_limit < self.delta_force_z or self.delta_force_z < -self.force_limit:
        	force_z_limit_r = - ( force_c * abs(abs(self.delta_force_z) - abs(self.force_limit)) * ( n_step - update ) * 0.1 + force_c )
                print("##3 force_z limit_r", force_z_limit_r)
        if self.torque_limit < self.delta_torque_x or self.delta_torque_x < -self.torque_limit:
        	torque_x_limit_r = - ( force_c * abs(abs(self.delta_torque_x) - abs(self.torque_limit)) * ( n_step - update ) * 0.1  + force_c)
        ...
        if self.torque_limit < self.delta_torque_y or self.delta_torque_y < -self.torque_limit:
        	torque_y_limit_r = - ( force_c * abs(abs(self.delta_torque_y) - abs(self.torque_limit)) * ( n_step - update ) * 0.1  + force_c)
        ...
        if self.torque_limit < self.delta_torque_z or self.delta_torque_z < -self.torque_limit:
        	torque_z_limit_r = - ( force_c * abs(abs(self.delta_torque_z) - abs(self.torque_limit)) * ( n_step - update ) * 0.1  + force_c)
        ...
        #7 release the knob(-)
        if self.min_static_taxel0 < self.min_static_limit and self.min_static_taxel1 < self.min_static_limit:
            min_static_limit_r = - ( taxel_c * (n_step - update) * 0.1 + taxel_c)
            print("##4 min_static limit_r", min_static_limit_r)
        if self.max_static_taxel0 > self.max_static_limit and self.max_static_taxel1 > self.max_static_limit:
            max_static_limit_r = - ( taxel_c * (n_step - update) * 0.1 + taxel_c)
            print("##4 max_static limit_r", max_static_limit_r)
        self.static_limit_r = min_static_limit_r + max_static_limit_r

                #8 joint(+)
        act_5_n_limit = 1.5709174034899702       # -0.005
        act_5_p_limit = 2.5709174034899702       #  1
        act_4_n_limit = -1.4670589583209175       # -0.005
        act_4_p_limit = -1.4370589583209175    #  0.03
        act_3_n_limit = 2.145370898415174  # -0.023
        act_3_p_limit = 2.168370898415174       #  0.005
        act_2_n_limit = 2.4377209990850974       # -0.005
        act_2_p_limit = 2.5777209990850974    #  0.14
        act_1_n_limit = -1.5596597816566892   # -0.11
        act_1_p_limit = -1.4496597816566892       #  0.005
        act_0_n_limit = 1.473122534496775  # -0.015
        act_0_p_limit = 1.488122534496775       #  0.005

        if act_5_n_limit < observation[5] and observation[5] < act_5_p_limit:
            action5_limit_r = observation[5] * act_5_n
            print("##5 action5 limit_r", action5_limit_r)
        if act_4_n_limit < observation[4] and observation[4] < act_4_p_limit:
            action4_limit_r = observation[4] * act_4_n
            print("##5 action4 limit_r", action4_limit_r)
        if act_3_n_limit < observation[3] and observation[3] < act_3_p_limit:
            action3_limit_r = -observation[3] * act_3_n
            print("##5 action3 limit_r", action3_limit_r)
        if act_2_n_limit < observation[2] and observation[2] < act_2_p_limit:
            action2_limit_r = observation[2] * act_2_n
            print("##5 action2 limit_r", action2_limit_r)
        if act_1_n_limit < observation[1] and observation[1] < act_1_p_limit:
            action1_limit_r = -observation[1] * act_1_n
            print("##5 action1 limit_r", action1_limit_r)
        if act_0_n_limit < observation[0] and observation[0] < act_0_p_limit:
            action0_limit_r = -observation[0] * act_0_n
        ...
        catesian_rpy_x = (1 - abs(2.57467815858803 - self.eef_rpy.x) * 10) * catesian_rpy_c
        catesian_rpy_y = (1 - abs(0.0145232756620397 - self.eef_rpy.y) * 10) * catesian_rpy_c
        catesian_rpy_z = (1 - abs(1.4799236454285 - self.eef_rpy.z) * 10) * catesian_rpy_c
        ...
        if self.cartesian_flag == 0:
            ...
            print("##7 cartesian_bonus_r", self.cartesian_bonus_r)
