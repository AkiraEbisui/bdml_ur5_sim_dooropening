...
#        rospy.logdebug("UR step func")	# define the logger
        ...
        for x in range(1, sub_step + 1):
            ...
            if self.moveit == 0:
                sub_action[0] = sub_action[0] / 40
                sub_action[1] = sub_action[1] / 90
                sub_action[2] = sub_action[2] / 60
                sub_action[3] = sub_action[3] / 120
                sub_action[4] = sub_action[4] / 20
...
#                    sub_action[5] =  2.575005366191597
                ...
                sub_action[2] = sub_action[2] / 10000
                sub_action[3] = sub_action[3] * 2
                sub_action[4] = sub_action[4] / 10000
...
# [1.527437940043594, -1.798838363862096, 2.137143238430859, -0.3239121423979796, 1.5586089455341092, -0.5865493562517852]
    ...
    def compute_dist_rewards(self, action, update):
        ...
        panel_c = 5000    #3 door panel open(+)
        ...
        elif 0.6 < self.imu_link_rpy.x < 0.7:
        ...
        elif 0.7 < self.imu_link_rpy.x:
            self.knob_rotation_r = 0.7 * knob_c + knob_bonus_c * 10                   # 0.7 * 30 + 10 * 10 = 121
            if (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) < 0.3:
                self.panel_rotation_r =  (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) * panel_c + panel_bounus_c
            elif 0.3 < (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) < 0.6:
                self.panel_rotation_r =  (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) * panel_c + panel_bounus_c * 2
            elif 0.6 < (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) < 0.9:
                self.panel_rotation_r =  (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) * panel_c + panel_bounus_c * 3
            elif 0.9 < (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) < 1.2:
                self.panel_rotation_r =  (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) * panel_c + panel_bounus_c * 4
            elif 1.2 < (self.imu_link_rpy_ini.z - self.imu_link_rpy.z):
                self.panel_rotation_r =  (self.imu_link_rpy_ini.z - self.imu_link_rpy.z) * panel_c + panel_bounus_c * 100
...
#        print("imu_link_rpy.z", self.imu_link_rpy_ini.z, self.imu_link_rpy.z)
        ...
        print("##1 door_rotation_r", self.panel_rotation_r)
