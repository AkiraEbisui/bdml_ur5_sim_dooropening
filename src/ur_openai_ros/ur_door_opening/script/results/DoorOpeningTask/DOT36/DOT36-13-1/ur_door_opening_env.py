...
#        print("min, max taxel", self.min_static_taxel0, self.max_static_taxel0, self.min_static_taxel1, self.max_static_taxel1)
    ...
    def compute_dist_rewards(self, action, update):
        ...
        act_0_n = 100       #8 action[0] (-)
        act_1_n = 100       #  action[1] (-)
        act_2_n = 100       #  action[2] (-)
        act_3_n = 100       #  action[3] (-)
        act_4_n = 100       #  action[4] (-)
        act_5_n = 100       #  action[5] (-)
        ...
        act_5_p_limit = 1       #  1
        act_4_n_limit = -0.005  # -0.005
        act_4_p_limit = 0.03    #  0.03
        act_3_n_limit = -0.023  # -0.023
        act_3_p_limit = 0.005   #  0.005
        act_2_n_limit = -0.005  # -0.005
        act_2_p_limit = 0.14    #  0.14
        act_1_n_limit = -0.11   # -0.11
        act_1_p_limit = 0.005   #  0.005
        act_0_n_limit = -0.015  # -0.015
        act_0_p_limit = 0.005   #  0.005

        #8 joint(+, -)
        action = action - self.arr_init_pos2
        if action[5] < act_5_n_limit:
            compute_rewards -= (act_5_n_limit - action[5]) * act_5_n
            print("action5 limit", compute_rewards)
        elif act_5_p_limit < action[5]:
            compute_rewards -= (action[5] - act_5_p_limit ) * act_5_n
            print("action5 limit", compute_rewards)
        if action[4] < act_4_n_limit:
            compute_rewards -= (act_4_n_limit - action[4]) * act_4_n
            print("action4 limit", compute_rewards)
        elif act_4_p_limit < action[4]:
            compute_rewards -= (action[4] - act_4_p_limit) * act_4_n
            print("action4 limit", compute_rewards)
        if action[3] < act_3_n_limit:
            compute_rewards -= (act_3_n_limit - action[3]) * act_3_n
            print("action3 limit", compute_rewards)
        elif act_3_p_limit < action[3]:
            compute_rewards -= (action[3] - act_3_p_limit) * act_3_n
            print("action3 limit", compute_rewards)
        if action[2] < act_2_n_limit:
            compute_rewards -= (act_2_n_limit - action[2]) * act_2_n
            print("action2 limit", compute_rewards)
        elif act_2_p_limit < action[2]:
            compute_rewards -= (action[2] - act_2_p_limit) * act_2_n
            print("action2 limit", compute_rewards)
        if action[1] < act_1_n_limit:
            compute_rewards -= (act_1_n_limit - action[1]) * act_1_n
            print("action1 limit", compute_rewards)
        elif act_1_p_limit < action[1]:
            compute_rewards -= (action[1] - act_1_p_limit) * act_1_n
            print("action1 limit", compute_rewards)
        if action[0] < act_0_n_limit:
            compute_rewards -= (act_0_n_limit - action[0]) * act_0_n
            print("action0 limit", compute_rewards)
        if act_0_p_limit < action[0]:
            compute_rewards -= (action[0] - act_0_p_limit) * act_0_n
            print("action0 limit", compute_rewards)
