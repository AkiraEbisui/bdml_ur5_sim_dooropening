...
nupdates1 = rospy.get_param("/ML/nupdates1")
nupdates2 = rospy.get_param("/ML/nupdates2")
test_range = rospy.get_param("/ML/test_range")
...
def sub_program(save_steps, env, test):
...
#                arr_x_data_fail_hist = np.array(x_data_fail_hist)
 ...
 #               arr_y_data_fail_hist = np.array(y_data_fail_hist) 
        ...
        if (update%1) == 0:
            ...
            if random_grasp >= 1:
                if env.success == 1:
                    ax14.hist(success_x_hist_list, facecolor='b') # array(-1.36089122)
                    ax15.hist(success_y_hist_list, facecolor='b')
                else:
                    ax14.hist(fail_x_hist_list, facecolor='r')
                    ax15.hist(fail_y_hist_list, facecolor='r')
            ax16.plot(x_data, y_data_success_rate, 'r-', linestyle="solid")
            ...
            if test == 0:
                ...
                plt.savefig('./results/' + sys.argv[1] + '/result/ppo_with_gae_list-' + str(save_steps) + '.png')

    if test == 0:
        with open('results/' + sys.argv[1] + '/sample.csv', 'w') as f0:        
            ...
            writer.writerow(success_x_hist_list) #53
            writer.writerow(success_y_hist_list) #54
            writer.writerow(fail_x_hist_list) #55
            writer.writerow(fail_y_hist_list) #56
            writer.writerow(y_data_success) #57
            writer.writerow(y_data_success_rate) #58
    elif test == 1:
        with open('results/' + sys.argv[1] + '/result/sample-' + str(save_steps) + '.csv', 'w') as f1:
...
def main():
    ...
    success_rate_sub0 = sub_program(0, env, 0)

    os.mkdir('./results/' + sys.argv[1] + '/result')
    for save_steps in reversed(range(test_range, nupdates1, test_range)):
        agent.saver.restore(agent.sess, './results/' + sys.argv[1] + '/ppo_with_gae_model-' + str(save_steps))
        env.success_times = 0
        success_rate_sub1 = sub_program(save_steps, env, 1)
        success_sub_list.append(success_rate_sub1)
        file_list.append(save_steps)
    with open('results/' + sys.argv[1] + '/result/' + sys.argv[1] + '_summary.csv', 'w') as f2:
        writer2 = csv.writer(f2)
        writer2.writerow(file_list) #1
        writer2.writerow(success_sub_list) #2
        print("Save_steps:", file_list)
        print("Success_rate_of_test:", success_sub_list)    

    print("######################  finish  ######################")
    print("Final_success_rate_of_training:", success_rate_sub0)
    print("######################  finish  ######################")
    print("Save_steps:", file_list)
    print("Success_rate_of_test:", success_sub_list)    
    print("######################  finish  ######################")
